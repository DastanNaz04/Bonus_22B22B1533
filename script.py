...
def clean_data(df):
    ...
    # УПРОЩЕННЫЙ ПОИСК столбца высоты: ищем любое имя, содержащее 'Высота'
    height_col_original = [col for col in cleaned_df.columns if 'Высота' in col]
    if height_col_original:
        renames[height_col_original[0]] = 'Height_Meters'
    
    # Поиск других столбцов
    for old_name_part, new_name in [('Ранг', 'Rank'), ('Название', 'Building_Name'), ('Город', 'City')]:
    ...
    # 2. ОПЕРАЦИЯ: Удаление ненужных столбцов
    cols_to_drop_keywords = ['фут', 'шпилю', 'крыше', 'постройки', 'Этажность']
    cols_to_drop = [
    ...
    # Извлекаем только число, стоящее первым (метры)
    cleaned_df['Height_Meters'] = cleaned_df['Height_Meters'].astype(str).str.split(' ').str[0]
    ...
    cleaned_df = cleaned_df.dropna(subset=['Height_Meters', 'Building_Name']) 
...
def json_serializer(data):
    """Сериализатор JSON для Kafka (обрабатывает NaN)."""
    def convert_nan(obj):
        if isinstance(obj, float) and pd.isna(obj):
            return None
        return obj
        
    return json.dumps(data, default=convert_nan).encode('utf-8')
